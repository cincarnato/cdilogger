...
//        $this->addListener($events->attach(MvcEvent::EVENT_FINISH, array($this, 'shutdown'), -1000));
        ...
        $this->listeners[] = $events->attach(MvcEvent::EVENT_FINISH, array($this, 'logResponse'));
        $this->listeners[] = $events->attach(MvcEvent::EVENT_FINISH, array($this, 'shutdown'), -1000);
    }
            ...
            $this->getLog()->debug(
                ...
                $event->getRequest()->getUri()->getHost() => array(
                    'Response' => array(
                        ...
                        'contentType' => (!$event->getResponse()->getHeaders()->get('Content-Type')) ? 'unknown' : $event->getResponse()->getHeaders()->get('Content-Type')->getMediaType(),
                        'content' => $content,
                    )
                )
                            )
